...
class Bot_Builders_node6 : public cSimpleModule
{
  private:
    int energy;
    int counter;
...
Define_Module(Bot_Builders_node6);

void Bot_Builders_node6::initialize()
{
    counter = par("limit");
    int src = par("source");
    if(getIndex() == src) {
...
void Bot_Builders_node6::handleMessage(cMessage *msg)
{
...
void Bot_Builders_node6::forwardMessage(frame_BB *msg)
{
...
frame_BB *Bot_Builders_node6::generateMessage()
{
    char frameName[20];
    int src = par("source");
